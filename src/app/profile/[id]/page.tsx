...
const LEVEL_THRESHOLDS = [
  ...
  { level: 5, name: '五水', min: 500, max: Infinity, color: 'from-rose-500 to-orange-500', badge: 'bg-rose-100 text-rose-800' },
]
...
export default function ProfileIdPage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const data = await authService.getProfileById(userId)
        if (!data) {
          toast.error("未找到该水分子！")
          router.push('/')
          return
        ...
        if (currentUser && currentUser.id === userId) {
          setIsOwner(true)
          // 仅所有者可以获取到邮箱，如果需要的话可以再次拉取完整信息
          const fullData = await authService.getUserProfile()
          if (fullData) setProfile(fullData)
        }
      } catch (error) {
  ...
  const handleSaveBio = async () => {
    ...
    if (trimmed.length > 100) {
      toast.error('个性签名最多 100 个字符')
      return
    }

    setIsSavingBio(true)
    try {
      await authService.updateBio(profile.id, trimmed)
          ...
          <div className="px-8 pb-10 relative">
            ...
            <div className="space-y-4 mb-10">
              ...
              <div className="ml-1 mt-2">
                {isEditingBio ? (
                  <div className="flex flex-col gap-2 mt-2 w-full sm:w-2/3">
                    <textarea
                      value={editBio}
                      ...
                      maxLength={100}
                      className="text-sm text-gray-700 border border-blue-400 rounded-lg p-3 outline-none focus:ring-4 focus:ring-blue-500/20 transition-all bg-white shadow-sm resize-none"
                      rows={3}
                      autoFocus
                    />
                    <div className="flex items-center gap-2 justify-end">
                      <span className="text-xs text-gray-400 mr-auto">{editBio.length}/100</span>
                      <button
                        onClick={handleCancelEditBio}
                        disabled={isSavingBio}
                        className="bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1 rounded-md font-bold shadow-sm transition-colors text-xs disabled:opacity-50"
                      >
                        取消
                      </button>
                      <button
                        ...
                        disabled={isSavingBio}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md font-bold shadow-sm transition-colors text-xs disabled:opacity-50"
                      >
                        {isSavingBio ? '保存...' : '保存'}
                      </button>
                    </div>
